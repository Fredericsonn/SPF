...
QUERIES="${QUERIES:-100}"
SEED="${SEED:-42}"
TRAVERSAL_CACHE_DIR="${TRAVERSAL_CACHE_DIR:-data/results/traversal_cache/paris_all_model}"
...
run_benchmark() {
  ...
  local mode="$3"
  local scale="$4"
  local input_file="data/subgraphs/paris/${group}_${test_id}.json.gz"
  local scale_dir="${scale//./_}"
  local output_file="data/results/ml_astar/paris_all_model/${group}/${mode}/scale_${scale_dir}/test_${test_id}.csv"

  echo
  echo "============================================================"
  echo "Benchmark: group=${group}, test=${test_id}, mode=${mode}, scale=${scale}"
...
run_group "paris_500" "009" "010"
run_group "paris_1000" "009" "010"
run_group "paris_2500" "009" "010"
run_group "paris_5000" "005"

echo
echo "Paris-all model benchmarks complete."
